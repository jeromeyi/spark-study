package org.shirdrn.spark.mllib
...
/**
 * hdfs dfs -mkdir /opt
	 hdfs dfs -put /opt/douban /opt/
 * 
 cd /data/spark-1.6.3-bin-hadoop2.6
./bin/spark-submit --class org.shirdrn.spark.mllib.DoubanRecommender --master yarn --deploy-mode cluster --executor-memory 2048m --total-executor-cores 2 /usr/local/spark-0.0.1-SNAPSHOT.jar
 * 
 */
